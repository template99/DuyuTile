// game logic goes here
    ...
    // needs rebuilding
    // 
    function totalTilesPerColor(thisColor){
        // grab a list of colors
        var ColorArray = [];
        var colorCounter = 0;
        // loop through each color
        tilecolors.forEach(element => {
            ColorArray[colorCounter] = getTotalTilesforColor(element);
        });
    }

    function getTotalTilesforColor(thisColor){
        var colorTotal = 0; 
        for (let row = 0; row < TOTALROWS; row++) {            
            for (let column = 0; column < TOTALCOLUMNS; column++) {
                //console.log(` RCR: ${column}${row}`);
                if (level.tiles[column][row].tileColor == thisColor)
                    {     
                       colorTotal+=1;
                    }
            }
    ...
    // check to see what we have for matches
    // before player plays turn
    function preCheckNeighbors(){
        // first, go by the color
        tileColors.forEach(element => {
            console.log(`(PC) Color: ${getTotalTilesforColor(1)}`);
